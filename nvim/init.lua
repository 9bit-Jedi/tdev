...
-- Personal
vim.cmd("iabbrev @@ utsah.iitr@gmail.com")
vim.cmd("iabbrev sig Utsah")
...
require("lazy").setup({
    spec = {
        ...
        { "rbong/vim-flog", cmd = { "Flog" }, dependencies = "tpope/vim-fugitive",
          keys = { { "<leader>gl", "<cmd>Flog<cr>", desc = "Git log graph" } } },

        {
            ...
            config = function()
                ...
                vim.keymap.set("n", "<leader>gwl", "<cmd>Telescope git_worktree git_worktrees<cr>", { desc = "Worktree: list/switch" })
                vim.keymap.set("n", "<leader>gwc", "<cmd>Telescope git_worktree create_git_worktree<cr>", { desc = "Worktree: create" })
            end,
        },

        {
            -- Jump to any visible location in 2-3 keystrokes — replaces `f`/`t`-then-hunt-and-peck and most `/` searches
            "folke/flash.nvim",
            event = "VeryLazy",
            opts = {},
            keys = {
        ...
        {
            -- Aggregated, navigable list of diagnostics / quickfix / LSP references
            "folke/trouble.nvim",
        ...
        {
            -- Which-key: surfaces all the leader-prefixed bindings above as a popup — essential once you add this many

...
if command -v fzf >/dev/null 2>&1; then
  # fd ships as `fdfind` on Debian/Ubuntu (name clash with another package)
  # and as `fd` on Arch — probe instead of guessing from the OS.
  _fd=$(command -v fdfind || command -v fd)
  if [[ -n "$_fd" ]]; then
    export FZF_DEFAULT_COMMAND="$_fd --type f --hidden --strip-cwd-prefix --exclude .git"
    export FZF_CTRL_T_COMMAND="$FZF_DEFAULT_COMMAND"
    export FZF_ALT_C_COMMAND="$_fd --type d --hidden --strip-cwd-prefix --exclude .git"
  fi
  unset _fd
